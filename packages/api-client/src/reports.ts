...
export function parsePersonalInsightsReport(content: unknown): PersonalInsightsReport | null {
  ...
  return {
    ...
    summary: {
      ...
      growthAreaLabel:
        typeof summary.growthAreaLabel === 'string' ? summary.growthAreaLabel : summary.growthAreaLabel ?? null,
      growthAreaScore:
        typeof summary.growthAreaScore === 'number' ? summary.growthAreaScore : summary.growthAreaScore ?? null,
      constraint: typeof summary.constraint === 'string' ? summary.constraint : summary.constraint ?? null,
    },
  };
}
...
export function buildPersonalInsightsReport(data: OnboardingData): PersonalInsightsReport {
  ...
  const growthSection = summary.growthAreaLabel
    ? {
        ...
        content: `The area asking for attention is **${summary.growthAreaLabel}**${
          typeof summary.growthAreaScore === 'number' ? `, currently at **${summary.growthAreaScore}/5**` : ''
  ...
  const sections = [valueSection, growthSection, constraintSection, reflectionSection].filter(Boolean) as PersonalInsightsSection[];
...
function isPersonalInsightsSection(value: unknown): value is PersonalInsightsSection {
  ...
  return typeof section.id === 'string' && typeof section.title === 'string' && typeof section.content === 'string';
}

function buildOpeningInsight(summary: PersonalInsightsSummary, sections: PersonalInsightsSection[]): string {
...
function buildReflectionSection(data: OnboardingData): PersonalInsightsSection {
  ...
  if (desire || avoidance) {
    return {
      ...
      content: [desire ? `**What you want:** ${desire}` : null, avoidance ? `**What you’re avoiding:** ${avoidance}` : null]
        .filter(Boolean)
        .join('\n\n'),
    };
  }
