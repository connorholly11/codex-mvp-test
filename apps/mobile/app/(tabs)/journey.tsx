...
import { fetchChatHistory, parsePersonalInsightsReport } from '@purpose/api-client';
...
export default function JourneyScreen() {
  ...
  const [reportSummary, setReportSummary] = useState<{ topValue: string | null; constraint: string | null }>({
    topValue: null,
    constraint: null,
  });
  ...
  useEffect(() => {
    ...
    void initializeQuests(accessToken);
  ...
  useEffect(() => {
    ...
    fetchChatHistory({ accessToken })
      .then((history) => {
        ...
        setUserMessageCount(messages.filter((message) => message.role === 'user').length);
      ...
      .catch((error) => {
        ...
        setChatError(error instanceof Error ? error.message : 'Unable to load chat insights.');
      })
      .finally(() => {
  ...
  const achievements = useMemo(
    () => [
      ...
      },
      {
        ...
        description: reportSummary.constraint ?? 'Name the pattern slowing you down to sharpen coaching.',
      },
    ],
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <View style={styles.section}>
            ...
            <Text style={styles.sectionTitle}>Track how Purpose evolves with you</Text>
            <Text style={styles.sectionBody}>
              We track momentum across quests, chats, and insights so you can see progress at a glance.
            </Text>
          </View>
          <View style={styles.metricsGrid}>
            {achievements.map((achievement) => (
              <View key={achievement.title} style={styles.metricCard}>
                ...
                <Text style={styles.metricDescription}>{achievement.description}</Text>
              </View>
            ))}
          </View>
          <View style={styles.section}>
            ...
            <Text style={styles.sectionBody}>
              Visit the Reports tab to read your Personal Insights in full. New summaries will appear here as we ship them.
            </Text>
